...
include_once('novnivoslider.php');

$context = Context::getContext();
$home_slider = new novnivoslider();
$slides = array();
...
if (Tools::getValue('action') == 'updateSlidesPosition' && Tools::getValue('slides'))
{
	...
	foreach ($slides as $position => $id_slide)
	{
		$res = Db::getInstance()->execute('
			UPDATE `'._DB_PREFIX_.'novnivoslider_slides` SET `position` = '.(int)$position.'
			WHERE `id_novnivoslider_slides` = '.(int)$id_slide
		);
