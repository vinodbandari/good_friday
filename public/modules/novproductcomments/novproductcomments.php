...
class NovProductComments extends Module
{
	...
	public function __construct()
	{
		...
		$this->displayName = $this->l('Vinova Product Comments');
	...
	public function install($keep = true)
	{
		...
		if (parent::install() == false ||
			...
			!$this->registerHook('displayProductDetailReviews') ||
	...
	public function uninstall($keep = true)
	{
		if (!parent::uninstall() || ($keep && !$this->deleteTables()) ||
			...
			!$this->unregisterHook('displayProductDetailReviews') ||
	...
	protected function _postProcess()
	{
		...
		elseif (Tools::isSubmit('novproductcomments'))
		{
		...
		elseif (Tools::isSubmit('deletenovproductcomments'))
		{
			$id_product_comment = (int)Tools::getValue('id_product_comment');
			$comment = new ProductComment($id_product_comment);
			$comment->delete();
		...
		elseif (Tools::isSubmit('deletenovproductcommentscriterion'))
		{
		...
		elseif (Tools::isSubmit('statusnovproductcommentscriterion'))
		{
			$criterion = new ProductCommentCriterion((int)Tools::getValue('id_product_comment_criterion'));
			if ($criterion->id)
			{
		...
		$this->_clearcache('novproductcomments_reviews.tpl');
	}
	...
	public function getContent()
	{
		...
		$this->_html  = '';
		if (Tools::isSubmit('updatenovproductcommentscriterion'))
	...
	public function renderCriterionList()
	{
		...
		$helper->toolbar_btn['new'] = array(
			'href' => $this->context->link->getAdminLink('AdminModules').'&configure='.$this->name.'&module_name='.$this->name.'&updatenovproductcommentscriterion',
	...
	public function hookdisplayProductListReviews($params)
	{
		...
		if (!$this->isCached('novproductcomments_reviews.tpl', $this->getCacheId($id_product)))
		{
			require_once(dirname(__FILE__).'/ProductComment.php');
		...
		return $this->display(__FILE__, 'novproductcomments_reviews.tpl', $this->getCacheId($id_product));
	}

	public function hookdisplayProductDetailReviews($params)
	{
		require_once(dirname(__FILE__).'/ProductComment.php');
		require_once(dirname(__FILE__).'/ProductCommentCriterion.php');
		...
		return ($this->display(__FILE__, '/novproductcomments-extra.tpl'));
	}

	public function hookProductTabContent($params)
	{
		//$this->context->controller->registerJavascript('modules-ratingpack', 'modules/'.$this->name.'/js/jquery.rating.pack.js', ['position' => 'bottom', 'priority' => 150]);
		...
		//$this->context->controller->registerJavascript('modules-novproductcomments', 'modules/'.$this->name.'/js/novproductcomments.js', ['position' => 'bottom', 'priority' => 150]);
		...
		$this->context->smarty->assign(array(
			...
			'nbComments' => (int)ProductComment::getCommentNumber((int)Tools::getValue('id_product')),
			'novproductcomments_controller_url' => $this->context->link->getModuleLink('novproductcomments'),
			'novproductcomments_url_rewriting_activated' => (int) Configuration::get('PS_REWRITING_SETTINGS'),
		...
		return ($this->display(__FILE__, '/novproductcomments.tpl'));
	}

	public function hookHeader()
	{
		$this->context->controller->addCSS($this->_path.'novproductcomments.css', 'all');
		...
		if (in_array($this->page_name, array('product', 'productscomparison')))
		{
			...
			$this->context->controller->addJS($this->_path.'js/novproductcomments.js');			
