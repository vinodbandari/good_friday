...
<div class="products_block nov-productlist product-tabs pt-0 {if isset($class) && $class}{$class}{/if}">
    <div id="{$tab}" class="block-product clearfix">
        <div class=" title-tab row align-items-center mb-30 block_nav">
            <div class="w-tab col-xl-5 col-lg-5 col-md-8 col-sm-8 col-xs-12 pl-0">
            ...
            <div class="col-xl-5 col-lg-5 col-md-4 col-sm-8 col-xs-12 text-md-right">
                {if isset($button) && !empty($button)}
                    <div class="mt-xs-30 mb-xs-15 pr-0 pr-xs-0">
                        ...
                        {if isset($categories) && $categories }
                            {foreach from=$categories item=category key=k name=categories}
                                <div class="tab-pane fade {if $smarty.foreach.categories.first }in active{/if}" id="{$tab}category{$k}" role="tabpanel">
                                    ...
                                    {if !empty($products)}
                                        <div id="{$name_tab nofilter}" class="nov-productslick product_list grid row spacing-{$spacing}" data-rows="{$number_row}" data-rows_mobile="{$number_row_mobile}"  data-autoplayTimeout="6000" data-loop="true" data-margin="30" data-arrows="{$show_arrows}" data-dots="{$show_dots}" data-autoplay="{$autoplay}" data-lg="{$lg}"  data-md="{$md}" data-xs="{$xs}">
                                            {include file='_partials/layout/items/item_five.tpl' number_row=$number_row attributes=$category.groups}
                                        </div>
                                    {else}
