...
//        }
    ...
    {
        $id_lang = (int) Context::getContext()->language->id;
        $sql = 'SELECT pl.name FROM ' . _DB_PREFIX_ . 'smart_blog_category_lang pl, ' . _DB_PREFIX_ . 'smart_blog_category p WHERE pl.id_smart_blog_category = p.id_smart_blog_category AND p.id_smart_blog_category = ' . pSQL($id) . ' AND pl.id_lang = ' . pSQL($id_lang);
        if (!$result = Db::getInstance()->executeS($sql))
            return false;
    ...
    public static function getCatLinkRewrite($id)
    {
        ...
        $sql = 'SELECT pl.link_rewrite FROM ' . _DB_PREFIX_ . 'smart_blog_category_lang pl, ' . _DB_PREFIX_ . 'smart_blog_category p
                       WHERE pl.id_smart_blog_category=p.id_smart_blog_category AND p.id_smart_blog_category=' . pSQL($id) . ' AND pl.id_lang = ' . pSQL($id_lang);
        if (!$result = Db::getInstance()->executeS($sql))
            return false;
